...
from schemas import RegisterUser,LoginUser,RoleAssignPermission,ForgetPassword,UserChangePassword
import uuid
from fastapi.responses import HTMLResponse
from fastapi.templating import Jinja2Templates
from pathlib import Path
from  email_notification.notify import send_registration_notification,send_reset_password_mail
...
async def change_password(change_password:UserChangePassword,user:User=Depends(User.current_user)):
    ...
    if not User.verify_password(change_password.old_password,user.password):      #check old password that i have enter with the password in the dataabase 
        raise ValueError(
...
def user_reset_password(request:Request,new_password:str =Form(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=500,detail=f"An unexcepted error occured .Report this message to support :{e}")



@app.get("/users/me/reset_password_template",
              response_class=HTMLResponse,
              summary="Reset password for a user", tags=["Users"])
def user_reset_password_template(request: Request, user: User = Depends(User.current_user)):
    """
    Resets password for a user.
    """
    try:
...
@app.post("/users/me/forgot_password",
              tags=["Users"])
async def user_forgot_password(request: Request, user_email: str):
    ...
    try:
        ...
        if user:
            access_token = Auth.generate_token(data=user_email, expire_minutes=TEMP_TOKEN_EXPIRE_MINUTES)
